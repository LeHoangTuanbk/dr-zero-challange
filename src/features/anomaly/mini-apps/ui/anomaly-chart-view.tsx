...
import type { AnomalyTask, FacilityAnomalyCount } from '@shared/data/mock/anomaly'
  ...
  <div className="flex flex-col gap-5 p-6">
    <div className="flex items-start justify-between">
      <div>
        ...
        <h2 className="text-lg font-semibold text-foreground">拠点別異常件数</h2>
      </div>
    ...
    <div className="rounded-xl border border-border bg-card p-4 h-52">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: -16 }} barSize={36}>
          <XAxis dataKey="facility_ja" tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false} />
          ...
          <Bar dataKey="anomaly_count" radius={[4, 4, 0, 0]}>
            {chartData.map((entry) => (
              <Cell
                ...
                fill={entry.facility_ja === task.facility_ja ? BAR_COLOR_ACTIVE : BAR_COLOR_DEFAULT}
              />
            ))}
          </Bar>
        </BarChart>
    ...
    <div className="rounded-lg border border-primary/20 bg-primary-50 px-4 py-2.5 flex items-center gap-3">
      ...
      <p className="text-sm text-foreground">
        <span className="font-semibold">{task.facility_ja}</span> で {facilityCount} 件の異常が検出されています
      </p>
    </div>
