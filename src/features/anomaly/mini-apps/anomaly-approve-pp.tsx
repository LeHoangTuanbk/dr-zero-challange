...
type ApproveState = "idle" | "confirming" | "done";

export function AnomalyApproveApp({
  context,
  onComplete,
  onBack,
  isReadOnly,
  ...
  const reviewOutput = context.stepOutputs["review"] as
    | {
        decision: string;
        finalValue: string;
  ...
  const from = reviewOutput?.originalValue ?? task.detected_value;
  const to = reviewOutput?.finalValue ?? "—";
  ...
  const handleApprove = () => {
    ...
    setTimeout(() => {
      setState("done");
      onComplete({ approved: true, finalValue: to, decision });
    }, 800);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-6 p-6">
        ...
        <div className="rounded-xl border border-green-200 bg-green-50 px-6 py-4 text-center">
          ...
          <p className="text-2xl font-bold text-green-800">{to}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-0">
            <div className="p-5">
              ...
              <p className="text-lg font-semibold text-red-600 line-through decoration-red-300">
                {from}
              </p>
            </div>
            ...
            <div className="p-5">
              ...
              <p className="text-lg font-bold text-green-700">{to}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {isDismiss && (
        <div className="rounded-xl border border-orange-200 bg-orange-50 p-5">
          ...
          <p className="text-sm text-orange-700">
            このタスクを却下すると、現在の値が維持されます。後から再検討することも可能です。
          </p>
        </div>
      )}
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className={cn(
              ...
              state === "confirming" && "opacity-70 cursor-not-allowed",
            )}
          >
            {state === "confirming" ? (
              <>
