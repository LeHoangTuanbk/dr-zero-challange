...
import { taskQueue, anomalyTasks, extractionTask, vendorTask } from '@shared/data/mock'
...
function buildContext(taskId: string, stepOutputs: Record<string, StepOutput>): WorkflowContext {
...
const NARRATIVES: Record<string, string> = anomalyNarratives

function pickNarrative(taskId: string, stepId: string): string | null {
  return NARRATIVES[`${taskId}:${stepId}`] ?? null
}

function makeNarrativeMsg(taskId: string, stepId: string, text: string): NarrativeMessage {
  return { id: `${taskId}:${stepId}:${Date.now()}`, taskId, stepId, text, timestamp: Date.now() }
}
  ...
  selectTask: (taskId) => {
    ...
    if (!workflowStates[taskId]) {
      ...
      set({
        ...
        workflowStates: {
          ...workflowStates,
          [taskId]: { currentStepId: firstStep.id, stepOutputs: {}, status: 'active' },
        },
