...
function StepPill({ step, index, status, onClick }: {
  step: StepConfig
  index: number
  status: StepStatus
  onClick?: () => void
  ...
  return (
    <button
      ...
      className={cn(
        ...
        status === 'done'    && 'bg-primary text-primary-foreground hover:bg-primary-700 cursor-pointer',
        status === 'active'  && 'bg-primary-50 text-primary ring-1 ring-primary/40',
...
export function WorkflowProgress({ steps, currentStepId, completedStepIds, onReviewStep }: WorkflowProgressProps) {
        ...
        <div key={step.id} className="flex items-center gap-1.5">
          ...
          {i < steps.length - 1 && (
            <div className={cn(
              ...
              completedStepIds.includes(step.id) ? 'bg-primary/40' : 'bg-border',
            )} />
          )}
        </div>
      ))}
